...
[GitHub Actions] ──► [YouTube Data API v3 (JP/Music急上昇 15件リアルタイム取得)]
        ...
        ├─► [3. MusicBrainz API] 30秒タイムアウト＋5秒リトライで ISWCコード ＆ 一次ソースURL取得
        ├─► [4. ファクト記録 ＆ 同名異曲検証ガード] 合否断定(OK/NG)を排し、取得コード・手動検索用キーワード・注意タグを記録
        ...
        └─► デジタル販売用CSV生成 (`public/downloads/viral_song_rights_master.csv`)
        │
【ビルド・即時公開】
...
### コア設計思想：ファクトベース（一次エビデンス）主義
1. **主観的断定（OK/NG）の排除**: システム側で軽率に「配信許諾OK」などの判定を下さず、「どの検索ワードで照合し、どのコード（ISWC）が取得できたか」という客観的事実（ファクト）およびエビデンスURLを記録・提示する。
2. **法的リスクの解消**: 誤判定による著作権侵害トラブルからユーザーおよび運営者を完全に保護し、最終判断をユーザー（クリエイター）に安全に委ねる構造。
3. **著作権と原盤権の分離管理**: 著作物（作詞・作曲）の識別情報を提供しつつ、CD音源等の原盤権侵害（アカウントBAN）を防ぐための「歌枠用カラオケ音源導線」を標準装備。
...
├── public/                         # 静的配信領域
│   ├── downloads/
│   │   └── viral_song_rights_master.csv  # ★【自動生成】一次エビデンス付き販売用CSV (UTF-8 BOM付き 19項目)
│   ├── llms.txt                    # ★【自動生成】GEO/AIクローラー向け構造化テキストガイド
│   └── robots.txt                  # AIクローラー許可設定
├── scripts/                        # バックエンドデータパイプライン
│   └── fetch_data.py               # ★【Gemini一括解析・iTunes照合・MusicBrainz ISWC・原盤権URL】全自動処理
├── src/                            # Astro フロントエンド
│   ├── data/
...
4. **MusicBrainz API 照合 ＆ 同名異曲検証**:
   * タイムアウト **30秒** ＆ 503負荷発生時の **5秒待機自動リトライ**。
   * 「タイトル優先検索」で最大10件取得後、Python側で演者キーワードとの合致をスマート検証。同名異曲の誤取得をシャットアウト。
5. **ファクト記録 ＆ 原盤権対策**:
   * コード照合ステータス（`ISWCコード確認済み` / `コード未取得 (JASRAC/NexTone等)`）および注意タグ（任天堂、ディズニー等）を客観記録。
   * 歌枠用カラオケ音源ダイレクト検索URL（`https://www.youtube.com/results?search_query=...`）を付与。
6. **マルチファイルエクスポート**:
   * マスターDB（`rights_master.json`）、表示用JSON（`songs.json`）、GEOガイド（`llms.txt`）、UTF-8 BOM付きCSV（`viral_song_rights_master.csv`）を一括出力。
...
* **データの性質**: 本システムが提供する情報は、公開APIおよび国際レジストリ等から機械的に集約したファクトデータです。
* **利用者の自己責任**: 楽曲の著作権管理状況（信託範囲や自己管理曲など）および原盤利用規約は変更される可能性があるため、利用者が商用配信やカバー制作を行う際は、必ず権利管理団体（JASRAC/NexTone等）および音源権利者の公式ポータルにて個別確認を行ってください。